...
def parseArguments():
    ...
    parser.add_argument('-ps', '--pyramid-scale', type=int, default=2, metavar='', required=False, help='Pyramid scale (Default: 2)')
...
def stitch_tiles(tiles_path, output_path):
    ...
    if nx != 1 and ny != 1:
        ...
        if overlap_x == 16 and overlap_y == 16:
            ...
            with open(os.path.join(output_path, 'flagged_images.csv'), 'a') as f:
                writer = csv.writer(f)
                writer.writerow([tiles_path, overlap_x, overlap_y])
    ...
    filename = os.path.join(output_path, uuid.uuid4().hex + '.dat')
            ...
            if type(results) is tuple:
                ...
                if pyramid_resolutions == 0:
                    ...
                    with tifffile.TiffWriter(outputImagePath, bigtiff=bigtiff) as tif:
                        ...
                        tif.write(images, subifds=num_pyramid_levels, **options)

                        # successively generate and save pyramid levels to the SubIFDs
                        for x in range(num_pyramid_levels):
                            
                                
                            tif.write(np.stack([cv2.resize(images[n, :, :], (floor(img_shape[2] * (1/pyr_scale)**(x+1)), floor(img_shape[1] * (1/pyr_scale)**(x+1))),interpolation=cv2.INTER_LINEAR) for n in range(len(images))]), subfiletype=1, **options)
